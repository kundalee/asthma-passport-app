...
Created under **Google Auth Platform → Clients → Create OAuth client ID**. Only iOS and Android clients were created — **no Web client**, since the app has no web frontend. That means the ID token's `aud` (audience) claim will be the iOS client ID on iOS and the Android client ID on Android; the backend must accept either as valid, instead of checking against one shared Web client ID.
...
| Android  | Asthma Passport Android | Package `com.example.asthma_passport_app`, SHA-1 of the local `~/.android/debug.keystore` | No code-side config needed — Android matches the client purely by package name + registered SHA-1(s) at runtime. |
...
- No `serverClientId` is passed to `GoogleSignIn()` — intentional, since there's no Web client (see §3).

## 6. What's left

- **Backend**: implement `POST /user/login/google`.
  - Request: `{ "id_token": "<Google ID token>" }`
  - Verify the token's signature against Google's public keys and accept either the iOS or Android client ID as a valid `aud` (pass both as an array to the verifier, since there's no single shared Web client ID — see §3). Google's own guide for this, with official library snippets per language (Node.js, Java, PHP, Python): https://developers.google.com/identity/sign-in/web/backend-auth#node.js
